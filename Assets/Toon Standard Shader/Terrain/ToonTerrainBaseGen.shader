...
Shader "Hidden/TerrainEngine/Splatmap/ToonStandard-BaseGen"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _Mask0_ST, _Mask1_ST, _Mask2_ST, _Mask3_ST;
            ...
            v2f vert(appdata_t v)
            {
                ...
                float2 uv = TRANSFORM_TEX(v.texcoord, _Control);
                o.texcoord0 = ComputeControlUV(uv);
                o.texcoord1 = TRANSFORM_TEX(uv, _Mask0);
                o.texcoord2 = TRANSFORM_TEX(uv, _Mask1);
                o.texcoord3 = TRANSFORM_TEX(uv, _Mask2);
                o.texcoord4 = TRANSFORM_TEX(uv, _Mask3);
                return o;
            }
            ...
            {
                float4 alpha = tex2D(_Control, i.texcoord0);
                float4 splat0 = tex2D(_Mask0, i.texcoord1);
                float4 splat1 = tex2D(_Mask1, i.texcoord2);
                float4 splat2 = tex2D(_Mask2, i.texcoord3);
                float4 splat3 = tex2D(_Mask3, i.texcoord4);

                float4 specular = splat0 * alpha.x;
                specular += splat1 * alpha.y;
                specular += splat2 * alpha.z;
                specular += splat3 * alpha.w;
                return specular;
            }
            ENDCG
        }
